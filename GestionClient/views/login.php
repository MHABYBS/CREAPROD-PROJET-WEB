          ...
          include '../core/clientC.php';
            if(isset($_POST['email']) && isset($_POST['password']))
            {
              if(!empty($_POST['email']) && !empty($_POST['password']))
              {
                 ...
                 $livreur=$userC->login("livreur",$mail,$pwd); 
                 $row1 = $userC->loginRow("admin",$mail,$pwd);
                 $row2 = $userC->loginRow("client",$mail,$pwd) ; 
                 $row3 = $userC->loginRow("livreur",$mail,$pwd) ;           
              ...
              if(!empty($client))
                {
                  foreach ($client as  $row) {
                    if($mail == $row['email'] && password_verify($pwd,$row['hashed_pwd']))
                    { 
                        //session_start();
                        $_SESSION['ID'] = $row['id'] ;
                  ...
                  if(!empty($livreur))
                {
                  foreach ($livreur as  $data) {
                    if($mail == $data['email'] && password_verify($pwd,$data['hashed_pwd']))
                    { 
                        //session_start();
                        $_SESSION['liv_cin'] = $data['cin'] ;
                        $_SESSION['liv_nom'] = $data['nom'] ;
                        $_SESSION['liv_prenom'] = $data['prenom'];
                        $_SESSION['liv_login']=$mail;
                        $_SESSION['liv_password']=$pwd;
                        $_SESSION['liv_role']="livreur";

                      ?>
                       <script type="text/javascript">
                         window.location="livreurIndex.php" ; // replace location by the target page
                       </script>
