...
try{		
  ...
  print_r($datas);
	...
	function rechercherreclam($id){
	    ...
	    function rechercherReclamation($search)
      {
         $sql="SELECT * FROM `reclamation` where `id` LIKE '%$search%'  OR `nom` like '%$search%' OR `prenom` like '%$search%'  OR `mail` like '%$search%' OR `raison` like '%$search%' OR `msg` like '%$search%' OR `date` like '%$search%'   " ;
         $db = config::getConnexion();
         try{
         $liste=$db->query($sql);
         return $liste;
